...
namespace NLEngine
{
	...
	public:
		virtual ~CType();
		CType(const std::string& name, const std::string& realname, const type_info& type_info_, size_t size) : name_(name), realname_(realname), type_info_(type_info_), size_(size), IsOrphaned(false) { anno_ = false; };
		CType(const std::string& name, const std::string& realname, const type_info& type_info_, size_t size, bool anno) : name_(name), realname_(realname), type_info_(type_info_), size_(size), anno_(anno), IsOrphaned(false) {};
		CType(const std::string& name, const std::string& realname, const type_info& type_info_, size_t size, bool anno, const std::vector<CTypeMetaBase*>& IN_Metas) : name_(name), realname_(realname), type_info_(type_info_), size_(size), anno_(anno), Metas(IN_Metas), IsOrphaned(false)
		{
			for (auto i : Metas)
			{
		...
		virtual void ApplyMeta(CTypeMetaBase* IN_Meta);
		virtual void ApplyMetas(const std::vector<CTypeMetaBase*>& IN_Metas);
		...
		bool GetIsOrphaned() const
		{
			return IsOrphaned;
		}

		template<typename MetaType>
		class TStaticMetaGetter
		{
			TStaticMetaGetter(MetaType* IN_Meta) :Meta(IN_Meta) {};
			MetaType* Meta = nullptr;
			static TStaticMetaGetter* Factory;
			static MetaType* GetMeta()
			{
				if (Factory == nullptr)
				{
					MetaType
						Factory = new TStaticMetaGetter<TStaticMetaGetter>();
				}
			}
		};
	protected:
		...
		size_t size_;
		std::vector<CTypeMetaBase*> Metas;
		bool IsOrphaned;
		bool anno_;//这个我忘记是啥了，但是又不能动
	};
	...
	template<typename InType>
	class TVariableTypeContainer
	{
	public:
		TVariableTypeContainer(InType& InType, bool IsConst) :Type_(InType), IsConst(IsConst) {};
		template<typename InTypei>
		TVariableTypeContainer(const TVariableTypeContainer<InTypei>& Type_container_) :Type_(Type_container_.Type_), IsConst(Type_container_.IsConst)
		{
		...
		bool InputRequire(const TVariableTypeContainer<OtherInType>& Other) const
		{
	...
	auto GetType_container()
	{
		...
		return TVariableTypeContainer(Type_, IsConst);
	};
