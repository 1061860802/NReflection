...
struct MyStructC : public MyStruct
{
    ...
    static void RegisterStruct()
    {
        using ValueType_ = typename VReflectionTraits::remove_const<typename VReflectionTraits::variable_traits<decltype(&MyStructC::ArrayTest)>::return_type>::type;
        auto& I = Register<MyStructC>().Info();
...
struct MyStructD : public MyStructC, public InterfaceTest
{
    ...
    static void RegisterStruct()
    {
        ...
        Register<MyStructD>().Info()
            .ApplyConstructor<>()
            .ApplyConstructor<const int&>()
            .ApplyVariable(&MyStructD::Mem, "Mem");
    }
};
...
int main()
{
    ...
    auto& T = GetType<MyStructD>();
    for (auto i : T.GetMemberVariablesIncludeParent())
    {
        std::cout << i->GetOwnerPtr()->GetTypeName()<<"::" << i->GetName()<<"   "<<i->GetTypeContainer().GetTypeConst().GetTypeName() << std::endl;
    }
    for (auto i : T.GetMemberFunctionsIncludeParent())
    {
        std::cout << i->GetOwnerPtr()->GetTypeName() << "::" << i->GetName() << "   " << i->GetType().GetTypeName() << std::endl;
    }
    for (auto i : T.GetStaticVariablesIncludeParent())
    {
        std::cout << i->GetOwnerPtr()->GetTypeName() << "::" << i->GetName() << "   " << i->GetTypeContainer().GetTypeConst().GetTypeName() << std::endl;
    }
    for (auto i : T.GetStaticFunctionsIncludeParent())
    {
        std::cout << i->GetOwnerPtr()->GetTypeName() << "::" << i->GetName() << "   " << i->GetType().GetTypeName() << std::endl;
    }
    MyStruct* CPtr = GetStaticType<MyStructC>()->Create<MyStruct>();
    float t = 0;
    T.GetMemberFunction("TestFunc1")->CallFunc(CPtr,t,12.0);
    auto array = T.GetMemberVariable("ArrayTest")->FromInstantGetVariablePtrConst<float[10]>(CPtr);
    for (size_t i = 0; i < 10; i++)
    {
        std::cout << (*array)[i] << std::endl;
    }
    auto val = T.GetMemberVariable("f1")->FromInstantGetVariablePtrConst<double>(CPtr);
