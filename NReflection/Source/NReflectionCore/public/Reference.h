...
{
public:
	TReference(const std::string& name, size_t size, const TVariable_Type_Container<CType> Inner) : CReferenceBase(name, RealTypeName<_InType_&>::Name,typeid(_InType_&), size, Inner) {};
...
{
public:
	RReference(const std::string& name, size_t size, const TVariable_Type_Container<CType> Inner) : CReferenceBase(name, RealTypeName<_InType_&&>::Name,typeid(_InType_&&), size, Inner) {};
	virtual void GCProcess(const void* OP_TargetObject, std::set<const void*>& OP_Inspecteds, std::set<IGCObjectInterface*>& OP_SomethingFound) const override
	{
		if (OP_Inspecteds.find(OP_TargetObject) != OP_Inspecteds.end()) return;
		CType::GCProcess(OP_TargetObject, OP_Inspecteds, OP_SomethingFound);
