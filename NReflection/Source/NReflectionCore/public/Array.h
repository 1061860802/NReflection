...
{
public:
	CArrayBase(const std::string& name, const std::string& realname, const type_info& type_info_,size_t size, size_t In_ArraySize,const TVariable_Type_Container<CType> Inner) : CType(name, realname,type_info_, size), ArraySize(In_ArraySize), Inner(Inner) 
	{
	};
...
{
public:
	TArray(const std::string& name, size_t size,const TVariable_Type_Container<CType> Inner) : CArrayBase(name,typeid(_InType_[In_ArraySize]).name(), typeid(_InType_[In_ArraySize]), size, In_ArraySize, Inner) 
	{
	};
