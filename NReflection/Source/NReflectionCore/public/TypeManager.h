...
namespace NLEngine
{
	...
	class NREFLECTIONCORE_API CTypeManager
	{
	private:
		class SpinLock {
		public:
	...
	public:
		CTypeManager();
		~CTypeManager();
		CType* FromNameFindTypeImp(const std::string& Name) const;
		CType* FromRealNameFindTypeImp(const std::string& Name) const;
		size_t TypesNum() const;
		template<typename OutType>
		OutType* FromNameFindType(const std::string& Name) const
		{
			Lock.lock();
			if (CType* tmp = FromNameFindTypeImp(Name))
			{
				Lock.unlock();
		...
		{
			Lock.lock();
			if (CType* tmp = FromRealNameFindTypeImp(Name))
			{
				Lock.unlock();
				return dynamic_cast<OutType*>(tmp);
			}
	...
	public:
		std::map<std::string, CType*> TypesMap;
		bool TryApplyType(CType* NewType);
	};

	NREFLECTIONCORE_API CTypeManager& GetTypeManager();

	template<typename T_InObjType>
	CType* GetObjRealType(T_InObjType* In_Target)
	{
		if constexpr (std::is_polymorphic_v<T_InObjType>)
		{
			CTypeManager& TypeManagerObj = GetTypeManager();
			CType* Result = TypeManagerObj.FromRealNameFindTypeImp(typeid(*In_Target).name());
			if (Result)
			{
				return Result;
			}
