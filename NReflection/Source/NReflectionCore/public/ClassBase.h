...
public:
	...
	virtual std::optional<Any> CreateAny(const std::vector<AnyRef> Params) const;
	template<typename T,typename... Params>
	T* Create(Params&&... args) const
	{
		std::optional<Any> tmp = ClassBase::CreateAny({args...});
		if (tmp)
		{
			const PointerBase* PointerType = dynamic_cast<const PointerBase*>(&tmp->GetValType().GetTypeConst());
			if (PointerType == nullptr) return nullptr;
			const ClassBase* Type = dynamic_cast<const ClassBase*>(&PointerType->GetInner().GetTypeConst());
			if (Type == nullptr) return nullptr;
			void* ptr = Type->CastTo(this, *(void**)(tmp->GetValue()));
	...
	void* CastTo(const ClassBase* TargetParentType, void* Self) const;
	const void* CastTo(const ClassBase* TargetParentType, const void* Self) const;
	bool IsBaseOf(const Type& other) const;
	virtual std::string print(const void* Ptr) const = 0;
protected:
	std::map<const ClassBase*,const ParentClassBase*> ParentClasses_;
