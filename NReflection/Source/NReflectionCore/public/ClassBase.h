...
public:
	virtual ~CClassBase();
	const std::vector<const CParentClassBase*>& GetDriectParents() const { return DirectParentClasses_; };
	const std::vector<CMemberVariableBase*>& GetLocalMemberVariables() const { return MemberVariable_; };
	const std::vector<CStaticVariableBase*>& GetLocalStaticVariables() const { return StaticVariable_; };
	const std::vector<CMemberFunctionContainerBase*>& GetLocalMemberFunctions() const { return MemberFunc_; };
	const std::vector<CStaticFunctionContainerBase*>& GetLocalStaticFunctions() const { return StaticFunc_; };
...
protected:
	...
	CClassBase(const std::string& Name, const std::string& realname, const type_info& type_info_, size_t size) :CType(Name, realname,type_info_,size) {}
	void ApplyMemberVariable(CMemberVariableBase* TMemberVariable)
	{
		MemberVariable_.push_back(TMemberVariable);
	}
};
