...
{
public:
	CReferenceBase(const std::string& IN_Name, const std::string& IN_RealName, const type_info& IN_TypeInfo,size_t IN_size, const TVariable_Type_Container<CType>& IN_Inner) : CType(IN_Name, IN_RealName,IN_TypeInfo, IN_size), Inner(IN_Inner)
	{
		return;
	};
