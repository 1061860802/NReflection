...
namespace NLEngine
{
	...
	{
	public:
		CPointerBase(const std::string& name, const std::string& realname, const type_info& type_info_, size_t size, const TVariableTypeContainer<CType> Inner) : CType(name, realname, type_info_, size), Inner(Inner) {};
		const TVariableTypeContainer<CType> Inner;
		const TVariableTypeContainer<CType> GetInner() const
		{
			return Inner;
		}
	...
	{
	public:
		TPointer(const std::string& name, size_t size, const TVariableTypeContainer<CType> Inner) : CPointerBase(name, typeid(_InType_*).name(), typeid(_InType_*), size, Inner) {};
