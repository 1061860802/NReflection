...
std::optional<Any> ClassBase::Create(const std::vector<AnyRef> Params) const
{
...
bool ClassBase::IsBaseOf(const Type& other) const
{
	for (size_t i = 0; i < DirectParentClasses_.size(); i++)
	{
		if (DirectParentClasses_[i]->GetTargetClass() == &other)
		{
			return true;
		}
		DirectParentClasses_[i]->GetTargetClass()->IsBaseOf(other);
	}
	return false;
}

void ClassBase::MembersGCProcess(const void* OP_TargetObject, std::set<const void*>& OP_Inspecteds, std::set<IGCObjectInterface*>& OP_SomethingFound) const
{
	for (size_t i = 0; i < DirectParentClasses_.size(); i++)
	{
		DirectParentClasses_[i]->GetTargetClass()->MembersGCProcess(DirectParentClasses_[i]->CastToParent(this), OP_Inspecteds, OP_SomethingFound);
	}
}
